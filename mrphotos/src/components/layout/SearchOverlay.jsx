...
export default function SearchOverlay({ isOpen, onClose }) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState([]); // Fetched available items
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (Array.isArray(data.galleries)) {
        setResults(data.galleries); // Display only available items
      } else {
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] min-w-[50px]">
        ...
        <div className="mt-5">
          {results.length > 0 ? (
            <ul className="text-white">
              {results.map((item, index) => (
                <SearchResults key={index} post = {item.title || item.name} />
